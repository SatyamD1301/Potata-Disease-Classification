...
import axios from "axios"; // FIX: Proper ES6 import
...
export const ImageUpload = () => {
  ...
  // 1. Handle File Preview and Memory Cleanup
  useEffect(() => {
    if (!selectedFile) {
  ...
  // 2. Handle API Call safely
  useEffect(() => {
    let isMounted = true; // Flag to track if component is active

    const sendFile = async () => {
      // Only proceed if we have a file and preview is ready
      if (image && selectedFile && preview) {
        ...
        try {
          ...
          // FIX: Stop loading only if component is still mounted
          if (isMounted) {
            setIsloading(false);
          }
        }
            ...
            <Card className={`${classes.imageCard} ${!image ? classes.imageCardEmpty : ''}`}>
              {image && <CardActionArea>
                <CardMedia
                  ...
                  image={preview}
                  component="img" // FIX: ensures image renders correctly
                  title="Potato Leaf"
                />
              </CardActionArea>
              }
